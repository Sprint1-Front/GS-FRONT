  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      fishRef.current.forEach(fish => {
        ...
        // rebate nas bordas
        if (fish.x < 3)  { fish.vx =  Math.abs(fish.vx); fish.x = 3  }
        if (fish.x > 97) { fish.vx = -Math.abs(fish.vx); fish.x = 97 }
        if (fish.y < 5)  { fish.vy =  Math.abs(fish.vy); fish.y = 5  }
        if (fish.y > 90) { fish.vy = -Math.abs(fish.vy); fish.y = 90 }
